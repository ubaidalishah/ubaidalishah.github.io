...
function loadTable() {
  ...
  if (storedData) {
    ...
    JSON.parse(storedData).forEach((entry) => {
      ...
      row.insertCell(
        1
      ).innerHTML = `<a href='${entry.website}' target='_blank'>${entry.website}</a>`;
      ...
      applyCell.innerHTML = entry.applied
        ? `<span class='tick' onclick='toggleApply(this)'>&#9989;</span>`
        : `<span class='cross' onclick='toggleApply(this)'>&#10060;</span>`;
...
function addRow() {
  ...
  if (name && website && country) {
    ...
    row.insertCell(
      1
    ).innerHTML = `<a href='${website}' target='_blank'>${website}</a>`;
